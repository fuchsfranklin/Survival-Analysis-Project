...
ui <- navbarPage(
  ...
  tabPanel(withMathJax(),
    ...
    navlistPanel(
      ...
      tabPanel(
        ...
        tags$ul(
          tags$li(withMathJax(
            tags$b("Survival Time Response", style = "font-size:18px;"),
            tags$br(),
          ...
          tags$li(
            withMathJax(
              tags$b("Censoring", style = "font-size:18px;"),
              tags$br(),
              ...
              tags$br(),
              tags$ul(
                tags$b("Types of Right-Censoring", style = "font-size:16px;"),
                tags$br(),
                tags$br(),
      ...
      tabPanel(
        "4. Non-Parametric Estimation of the Survival Function",
      ...
      tabPanel(
        "5. Survival Probability Confidence Interval Estimates",
      ...
      tabPanel(
        "6. The Hazard Function and Cumulative Hazard",
      ...
      tabPanel(
        "7. The Chi-Squared Distribution and Test",
  ...
  ###################################################################
  tabPanel(
    title = "A Covid Related Application Example",
...
server <- function(input, output) {
  ...
  output$vtout3sub <- renderPrint({
    cat("## ",
      ...
      "## X-squared = 1045.3, df = 1, p-value < 2.2e-16",
      sep = "\n"
    )
  })
  ...
  output$vtout5 <- renderPrint({
    cat("lf_table <- survfit(Surv(EventTimePrimaryD57, EventIndPrimaryD57)~1, data = data_sub) ",
      "summary(lf_table, censored = TRUE)",
      sep = "\n"
    )
  })
  ...
  censor_plot <- ggplot() +
    ...
    theme_minimal() +
    labs(
      title = "Right-Censored Survival Data",
      color = "Censoring"
    ) +
    xlab("Subject Number") +
    ylab("Number of Years")

  output$censor_plot <- renderPlotly({
    ggplotly(censor_plot)
  })
